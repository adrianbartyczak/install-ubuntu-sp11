...
2. Edit the following file with nano or vim: > sudo nano /etc/apt/apt.conf.d/20auto-upgrades
...
4. Run script "fix-bluetooth-firmware.sh".

5. Run script "fix-wifi-firmware.sh".
...
7. Check if the Wi-Fi toggle is working. If not, please run "debug-wifi-firmware.sh". It will tell you what
   ...
   It may also be enough just to remove the Wi-Fi Udev rule in /etc/udev/rules.d/99-fix_mac_addresses.rules (which
   contains both a Wi-Fi and bluetooth Udev rule) and then reboot.

8. Check if Bluetooth is working. If not, run "fix-bluetooth-firmware-method-2.sh", and
   reboot.

   This method uses the btmgmt command and EFI variables to assign the bluetooth MAC address at boot.
   The first fix-bluetooth-firmware-method.sh script uses udev rules and is based on the following tutorial:
...
Thanks to @kyjus25 for finding the solution to fixing Wi-Fi and Bluetooth (via EFI assignment method) on the Surface Pro 11.
...
- <https://github.com/adrianbartyczak/install-ubuntu-sp11/blob/main/other-data/documents/battery-comparison-tests.txt>
...
1. Run the following command in PowerShell:
   ```bash
   set-vmvideo -vmname <virtual_machine_name> -horizontalresolution:<width> -verticalresolution:<height> -resolutiontype single
   ```

2. Start or restart your virtual machine.

3. In your virtual machine, get your display name by running the following command:
   ```bash
   xrandr 
   ```

(The display name should be the first column text on the second line [e.g. Virtual-1])  

4. Set your custom resolution by running the following commands:
   ```bash
   cvt <width> <height> <fps>
   xrandr --newmode <modeline_string_(all_text_after_"Modeline"_from_cvt_command)>
   xrandr --addmode <display_name> "<width>x<height>_<fps>.00"
   xrandr --output <display_name> --mode "<width>x<height>_<fps>.00"
   ```
